...
%  Run this script to perform IBVS
%%
clear all; clc;
close all;
%%/////////////////////////////////////////////////////////////////////////
...
%%/////////////////////////////////////////////////////////////////////////
%% Describing Camera Intrinsic Matrix below
f_x = 800; % focal length (mm) / pixel_width_x (mm) = pixels 
f_y = 800; % focal length (mm) / pixel_height_y (mm) = pixels
uox = 512; % image center x-coordinate
voy = 512; % image center y-coordinate
...
%theta_AR = -130*deg2radi;   % losing 2 feature (2 points)
...
t_AR = [ 0.0; 0.0; 0.9 ];  % normal condition
...
s_B = inv(Kin) * [p1B, p2B, p3B, p4B ; 1 1 1 1];
...
%Lxd=[];
    ...
    Lxd = [];
      % desired value of the interaction matrix
    for i=1:4
        
         Z= P_B(3,i);
        Lxd_temp = [ -1/Z, 0,    s_B(1,i)/Z, s_B(1,i)*s_B(2,i), -( 1 + s_B(1,i)^2), s_B(2,i);
                    0,    -1/Z, s_B(2,i)/Z, 1 + s_B(2,i)^2,      -s_B(1,i)*s_B(2,i), -s_B(1,i)];
...
for t=0:dt:tf 
   ...
   %%/////////////////////////////////////////////////////////////////////////
    %% Building the point features vector s_A in metric units now
    s_A  = inv(Kin) * [p1A, p2A, p3A, p4A ; 1 1 1 1];

    %% Calculate interaction matrix now
    Z = mean(P_B(3,:));
    Lx = [];
      % current value of the interaction matrix
    for i=1:4
    ...
    %% Plotting the 3D scene wrt to the reference frame at R  
    %clf;
